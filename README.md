# skypro-kanban
Чек-лист:
1. В приложении есть как минимум компоненты: Calendar, Main, Column, Header, PopBrowse, PopNewCard, PopUser, Register, Login.
Компоненты лежат в папке /src/components, для каждого отдельного компонента создана своя директория.
2. Модальное окно отображается при нажатии на имя пользователя.
3. Все колонки с карточками отрисованы согласно макету.
4. Реализована функция добавления новой задачи в столбец «Без статуса».
5. Создано состояние для отслеживания загрузки данных. Реализовано отображение текста «Загрузка» во время загрузки. После окончания загрузки данные о карточках отображаются на экране.
6. Необходимые для проекта хуки/контексты/массивы/глобальные стили/страницы находятся в своих папках.
7. Интерфейс страницы с задачами стилизован корректно, верстка соответствует макету. Цветовое оформление тем в карточках товаров соответствует макету. 
8. Стилизованные компоненты расположены в отдельных файлах. В проекте файл с расширением css ( закомментирован)
9. Создана маршрутизация по страницам.
10. Если пользователь не авторизован, то при попытке перехода на защищенные страницы («Доска с карточками», «Просмотр карточки», «Выход из аккаунта») приложение перенаправляет пользователя на страницу логина. Переходы между страницами работают без перезагрузки приложения.
...
21. Корректно реализована логика работа с api, работает индикатор загрузки, ошибки обрабатываются и выводятся пользователю.
22. Приложение работает без ошибок в консоли ( за исключением предупреждения Connect(Droppable))
23. Создано редактирование и удаление задачи и отправка данных в API.
24. Для перемещения карточек использована библиотека React Beautiful DnD. Перемещается одна карточка в завсимости от колонки. 